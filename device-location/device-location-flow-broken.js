...
const BASE_URL = 'https://sandbox.opengateway.telefonica.com/apigateway'        // Usar coordenadas predefinidas para prueba rápida
        console.log('\n🗺️ Usando coordenadas predefinidas para prueba rápida:');
        console.log('   📍 Madrid: 40.4168, -3.7038');
        console.log('   🎯 Precisión: 10 km');
        console.log('');
        
        const latitude = 40.4168;
        const longitude = -3.7038;
        const accuracy = 10;PE = 'dpv:FraudPreventionAndDetection#device-location-read';
...
async function verifyDeviceLocation(accessToken, phoneNumber, latitude, longitude, accuracy, uePort = null) {
    ...
    const requestBody = {
        ueId: {
            externalId: "testuser@domain.com",  // Email format as required
            msisdn: phoneNumber,                 // Phone number 
            ipv4Addr: "192.168.0.1/24",        // IPv4 address with mask
            ipv6Addr: "2001:db8:85a3:8d3:1319:8a2e:370:7344"  // IPv6 address
        },
        latitude: latitude,
        longitude: longitude,
        accuracy: accuracy
    ...
    // No agregar uePort ya que no está permitido según el error
...
async function main() {
    try {
        ...
        // Usar coordenadas predefinidas para prueba rápida
        console.log('\n🗺️ Usando coordenadas predefinidas para prueba rápida:');
        console.log('   📍 Madrid: 40.4168, -3.7038');
        console.log('   🎯 Precisión: 10 km');
        console.log('   � Puerto: 5060 (ejemplo)');
        console.log('');
        ...
        const result = await verifyDeviceLocation(accessToken, phoneNumber, latitude, longitude, accuracy);
